...
const float EnemyCrawler::_radius = 4.0f;

EnemyCrawler::Stats::Stats()
...
void EnemyCrawler::Assets::load() {
    _crawlerBodyWalk.loadFromFile("resources/enemies/crawler/crawler_body_walk.png");
    _crawlerBodyNoFire.loadFromFile("resources/enemies/crawler/crawler_body_nofire.png");

    _crawlerBodyFlashes.resize(3);

    _crawlerBodyFlashes[0].loadFromFile("resources/enemies/crawler/crawler_body_fire_flash1.png");
    _crawlerBodyFlashes[1].loadFromFile("resources/enemies/crawler/crawler_body_fire_flash2.png");
    _crawlerBodyFlashes[2].loadFromFile("resources/enemies/crawler/crawler_body_fire_flash3.png");

    _crawlerFoot.loadFromFile("resources/enemies/crawler/crawler_foot.png");

    _crawlerShadow.loadFromFile("resources/enemies/crawler/crawler_shadow.png");

    _crawlerSelected.loadFromFile("resources/enemies/crawler/crawler_selected.png");
...
void EnemyCrawler::create() {
    if (!getGame()->_resources.exists("enemy_crawler_assets")) {
        ...
        getGame()->_resources.insertPtr("enemy_crawler_assets", assets);
    }
    else
        _assets = getGame()->_resources.getPtr<Assets>("enemy_crawler_assets");

    if (!getGame()->_resources.exists("enemy_crawler_stats")) {
        Ptr<Stats> stats = std::make_shared<Stats>();

        _stats = stats;

        getGame()->_resources.insertPtr("enemy_crawler_stats", stats);
    }
    else
        _stats = getGame()->_resources.getPtr<Stats>("enemy_crawler_stats");
...
void EnemyCrawler::setPosition(const sf::Vector2f &position) {
    _position = position;
}

void EnemyCrawler::setRotation(float rotation) {
    _rotation = rotation;
}

void EnemyCrawler::move(sf::Vector2f &position) {
    _attack = false;
    _target = position;
    _pTarget = nullptr;
    _hold = false;
...
void EnemyCrawler::attackMove(const sf::Vector2f &position) {
    _attack = true;
    _target = position;
    _hold = false;
}

void EnemyCrawler::split() {
    _attack = false;
    _target = _position;
...
void EnemyCrawler::face(float &angle, float rate, float dt) {
    if (angle < 0.0f)
        angle += 360.0f;
    ...
    if (_pTarget != nullptr) {
        ...
        if (dist > _stats->_range)
            ...
            if (_firingCycle >= 1.0f) {
                ...
                _currentFlash = (_currentFlash + 1) % _assets->_crawlerBodyFlashes.size();
            }
            else
...
void EnemyCrawler::subUpdate(float dt, int subStep, int numSubSteps) {
...
void EnemyCrawler::preRender(sf::RenderTarget &rt) {
    if (_isSelected) {
        sf::Sprite selectedSprite;
        selectedSprite.setTexture(_assets->_crawlerSelected);

        selectedSprite.setOrigin(_assets->_crawlerSelected.getSize().x * 0.5f, _assets->_crawlerSelected.getSize().y * 0.5f);
...
void EnemyCrawler::render(sf::RenderTarget &rt) {
    sf::Sprite shadowSprite;
    shadowSprite.setTexture(_assets->_crawlerShadow);

    shadowSprite.setOrigin(_assets->_crawlerShadow.getSize().x * 0.5f, _assets->_crawlerShadow.getSize().y * 0.5f);
    ...
    footSprite.setTexture(_assets->_crawlerFoot);
    ...
    if (_pTarget == nullptr) {
        bodySprite.setTexture(_assets->_crawlerBodyWalk);
        ...
        if (_firingCycle < 0.3f) {
            ...
            bodySprite.setTexture(_assets->_crawlerBodyFlashes[_currentFlash]);
        }
        else {
            bodySprite.setPosition(_position);
            bodySprite.setTexture(_assets->_crawlerBodyNoFire);
        }
    }

    bodySprite.setOrigin(sf::Vector2f(_assets->_crawlerBodyWalk.getSize().x * 0.5f, _assets->_crawlerBodyWalk.getSize().y * 0.5f));
