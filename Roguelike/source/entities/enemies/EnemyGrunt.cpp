...
const float EnemyGrunt::_radius = 4.0f;

EnemyGrunt::Stats::Stats()
...
void EnemyGrunt::Assets::load() {
	_gruntBodyWalk.loadFromFile("resources/enemies/grunt/grunt_body_walk.png");
	_gruntBodyNoFire.loadFromFile("resources/enemies/grunt/grunt_body_nofire.png");

	_gruntBodyFlashes.resize(3);

	_gruntBodyFlashes[0].loadFromFile("resources/enemies/grunt/grunt_body_fire_flash1.png");
	_gruntBodyFlashes[1].loadFromFile("resources/enemies/grunt/grunt_body_fire_flash2.png");
	_gruntBodyFlashes[2].loadFromFile("resources/enemies/grunt/grunt_body_fire_flash3.png");

	_gruntFoot.loadFromFile("resources/enemies/grunt/grunt_foot.png");

	_gruntShadow.loadFromFile("resources/enemies/grunt/grunt_shadow.png");

	_gruntSelected.loadFromFile("resources/enemies/grunt/grunt_selected.png");
...
void EnemyGrunt::create() {
	if (!getGame()->_resources.exists("enemy_grunt_assets")) {
		...
		getGame()->_resources.insertPtr("enemy_grunt_assets", assets);
	}
	else
		_assets = getGame()->_resources.getPtr<Assets>("enemy_grunt_assets");

	if (!getGame()->_resources.exists("enemy_grunt_stats")) {
		Ptr<Stats> stats = std::make_shared<Stats>();

		_stats = stats;

		getGame()->_resources.insertPtr("enemy_grunt_stats", stats);
	}
	else
		_stats = getGame()->_resources.getPtr<Stats>("enemy_grunt_stats");
...
void EnemyGrunt::setPosition(const sf::Vector2f &position) {
	_position = position;
}

void EnemyGrunt::setRotation(float rotation) {
	_rotation = rotation;
}

void EnemyGrunt::move(sf::Vector2f &position) {
	_attack = false;
	_target = position;
	_pTarget = nullptr;
	_hold = false;
...
void EnemyGrunt::attackMove(const sf::Vector2f &position) {
	_attack = true;
	_target = position;
	_hold = false;
}
...
void EnemyGrunt::face(float &angle, float rate, float dt) {
	if (angle < 0.0f)
		angle += 360.0f;
...
void EnemyGrunt::preRender(sf::RenderTarget &rt) {
	if (_isSelected) {
		sf::Sprite selectedSprite;
		selectedSprite.setTexture(_assets->_gruntSelected);

		selectedSprite.setOrigin(_assets->_gruntSelected.getSize().x * 0.5f, _assets->_gruntSelected.getSize().y * 0.5f);
