...
void Marine::split() {
	...
	for (int i = 0; i < occupants.size(); i++) {
		...
		if (pEntity != this && (pEntity->_type == 1)) {	
...
void Marine::face(float &angle, float dt) {
	if (angle < 0.0f)
		angle += 360.0f;
	...
	if (minAngle <= _stats->_spinRate * dt) {
		_rotation = angle;
	}
	else {
		if (rotateDir < 0.0f)
			_rotation += -_stats->_spinRate * dt;
		else
			_rotation += _stats->_spinRate * dt;
